...
- **安全**：Express 仅监听 loopback，CSP/X-Frame-Options/Permissions-Policy 已配置，API 速率限制已启用，Hugo Goldmark `unsafe = false`
...
- **无自动生成内容**：所有元数据（summary、tags、categories）来自 front matter。缺失时默认为空——不从正文推断。
...
- **Hugo 主题**：`PaperMod`（见 `hugo.toml`），存储在 gitignored 的 `themes/` 目录。自定义布局：搜索页（`discovery-search`）、taxonomy 列表页/详情页。
