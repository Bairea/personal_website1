...
基于 Hugo 构建的中文个人知识网站。提供文章发布、分类浏览、客户端搜索（Pagefind）。
...
| 命令 | 作用 |
|---|---|
| `npm run build` | Hugo 构建 + Pagefind 搜索索引 |
...
推送到 GitHub 后 Vercel 自动构建部署。构建命令：
...
2. 直接使用系统级二进制（推荐，零 Node.js 依赖）：
   ```bash
   hugo --cleanDestinationDir
   pagefind --site public --force-language zh
   ```

**服务**：

Nginx 直接服务 `public/` 目录。配置示例见 `deploy/nginx-personal-website.conf`。
